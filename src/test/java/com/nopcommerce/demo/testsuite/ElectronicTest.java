...
public class ElectronicTest extends TestBase {
    ...
    public void verifyThatTheProductAddedSuccessfullyAndPlaceOrderSuccessfully() throws InterruptedException {
       ...
       electronicsPage.clickOnAddTocart();
        ...
        electronicsPage.clickonClose();
        //	2.11 Then MouseHover on "Shopping cart" and Click on "GO TO CART" button.
        electronicsPage.mousehoveronShoppingCart();
        Thread.sleep(2000);
        electronicsPage.clickOnGoToCart();
       //2.12 Verify the message "Shopping cart"
        Assert.assertEquals(electronicsPage.verifyTextShoppingCart(), "Shopping cart", "not matched");
        //	2.13 Verify the quantity is 2
        System.out.println("Check: "+electronicsPage.verifyQuntityTwo());
        String aQTY=electronicsPage.verifyQuntityTwo();
        Assert.assertEquals(aQTY, "2", "not matched");
        //2.14 Verify the Total $698.00
        Thread.sleep(2000);
        Assert.assertEquals(electronicsPage.verifyTotal(), "$698.00", "not matched");
        //2.15 click on checkbox “I agree with the terms of service”
        electronicsPage.clickOnTerms();
